...
// 세션 시작
    ...
    header('Content-Type: application/json'); // 응답 데이터 타입을 JSON으로 설정
    ...
    function get_recommendations($food_name, $foods, $cosine_sim) {
        ...
        // 자기 자신을 제외한 5개의 추천 음식을 슬라이싱
        $selected_result[0] = $result[1];
        $selected_result[1] = $result[2];
        $selected_result[2] = $result[50];
        $selected_result[3] = $result[75];
        $selected_result[4] = $result[100];

        // 추천 음식 목록 5개의 인덱스 정보 추출
        $food_indices = array();
    ...
    $food_data = getFoodImage($food_names);

    echo json_encode($food_data);  // 배열을 JSON 형식으로 변환하여 출력
?>